var UseList = [
  50676, 53150, 93133, 93134, 93135, 93767, 4100030, 4090004, 4090005,
];
var RareMineralogyList = [64041, 64054, 64055, 64056, 67100, 95310];
var SulienEcologyList = [51101, 51102, 51106, 51109, 67200, 95309];
var FindMaterialList = [
  50465, 50466, 50467, 50468, 50469, 50470, 50471, 50472, 50473, 50474, 50475,
  50476, 50477, 50478, 50479, 50480, 50483, 50484, 50485, 50486, 50487, 50488,
  50489, 50490, 51300, 51301, 60023, 60024, 60025, 60026,
];
var EffectName = [
  "持续时间",
  "最大生命值",
...
function CloseItemList(Id) {
  let ItemListHeight = parseInt(
    document.getElementById("ItemList").clientHeight,
  );
  if (ItemListHeight > 10) {
    document.getElementById("ItemList").style.height =
      ItemListHeight - 10 + "px";
...
function Item(Id, a) {
  ...
  try {
    ...
    for (var i = 0; i < EventMillingList.length; i++) {
      if (EventMillingList[i].includes(Id))
        EventLimit += EventMillingList[i][0];
    }
    t += "</tr><tr>" + TdSkill(10012, QuestLimit, LocaleLimit, EventLimit);
    t +=
      "<td width='580'>" +
      CompleteTable(
        TdMaterial(MillingItem[0], "×" + MillingItem[1], 160),
        560,
      ) +
      "</td>";
    EventLimit = "";
  ...
  try {
    ...
    for (var i = 1; i < TailoringItem.length; i++) {
      ...
      for (var j = 1; j < TailoringItem[i].length; j += 2) {
        ...
        tt += TdMaterial(
          TailoringItem[i][j],
          "×" + TailoringItem[i][j + 1] + AmendQuantity,
        );
      }
  ...
  try {
    ...
    for (var i = 0; i < EventHandicraftList.length; i++) {
      if (EventHandicraftList[i].includes(Id))
        EventLimit += EventHandicraftList[i][0];
    }
    ...
    for (var i = 1; i < HandicraftItem.length; i++) {
      ...
      for (var j = 0; j < HandicraftItem[i].length; j += 2) {
        tt += TdMaterial(
          HandicraftItem[i][j],
          "×" + HandicraftItem[i][j + 1],
          560 / HandicraftItem[i].length,
        );
      }
      t += "<td width='580'>" + CompleteTable(tt, 560) + "</td>";
    }
  ...
  try {
    ...
    if (TalentBlacksmithList.includes(Id))
      TalentLimit = TalentBlacksmithList[0];
    ...
    for (var i = 1; i < BlacksmithItem.length; i++) {
      ...
      for (var j = 1; j < BlacksmithItem[i].length; j += 2) {
        ...
        tt += TdMaterial(
          BlacksmithItem[i][j],
          "×" + BlacksmithItem[i][j + 1] + AmendQuantity,
        );
      }
      t += "<td width='580'>" + CompleteTable(tt, 560) + "</td>";
    }
  ...
  try {
    ...
    for (var i = 0; i < EventCookingList.length; i++) {
      if (EventCookingList[i].includes(Id))
        EventLimit += EventCookingList[i][0];
    }
    t +=
      "</tr><tr>" +
      TdSkill(
    ...
    for (var i = 1; i < Length; i += 6) {
      ...
      t +=
        "<td width='580'>" +
        CompleteTable(
          TdMaterial(CookingItem[i], CookingItem[i + 1] + "%", 160) +
            TdMaterial(CookingItem[i + 2], CookingItem[i + 3] + "%", 160) +
            TdMaterial(CookingItem[i + 4], CookingItem[i + 5] + "%", 160),
          560,
        ) +
        "</td>";
    }
  ...
  try {
    ...
    for (var i = 0; i < EventPotionMakingList.length; i++) {
      if (EventPotionMakingList[i].includes(Id))
        EventLimit += EventPotionMakingList[i][0];
    }
  ...
  if (tt != "")
    t += "</tr><tr>" + TdSkill(10023, "", "", EventLimit) + TdText(tt);
  EventLimit = "";
  try {
  ...
  try {
    ...
    t +=
      ...
      TdSkill(
        ...
        EventLimit,
        TalentLimit,
        StageTicketMakingItem.length - 1,
      );
    for (var i = 1; i < StageTicketMakingItem.length; i++) {
      ...
      for (var j = 0; j < StageTicketMakingItem[i].length; j += 2) {
        tt += TdMaterial(
          StageTicketMakingItem[i][j],
          "×" + StageTicketMakingItem[i][j + 1],
          560 / StageTicketMakingItem[i].length,
        );
      }
      t += "<td width='580'>" + CompleteTable(tt, 560) + "</td>";
    }
  ...
  try {
    ...
    t +=
      ...
      TdSkill(
        ...
        EventLimit,
        TalentLimit,
        HeulwenEngineeringItem.length - 1,
        HeulwenEngineeringItem[0],
      );
    for (var i = 1; i < HeulwenEngineeringItem.length; i++) {
      ...
      for (var j = 0; j < HeulwenEngineeringItem[i].length; j += 2) {
        tt += TdMaterial(
          HeulwenEngineeringItem[i][j],
          "×" + HeulwenEngineeringItem[i][j + 1],
          560 / HeulwenEngineeringItem[i].length,
        );
      }
      t += "<td width='580'>" + CompleteTable(tt, 560) + "</td>";
    }
  ...
  try {
    ...
    for (var i = 1; i < MagicCraftItem.length; i++) {
      ...
      for (var j = 0; j < MagicCraftItem[i].length; j += 2) {
        tt += TdMaterial(
          MagicCraftItem[i][j],
          "×" + MagicCraftItem[i][j + 1],
          560 / MagicCraftItem[i].length,
        );
      }
      t += "<td width='580'>" + CompleteTable(tt, 560) + "</td>";
    }
  ...
  try {
    ...
    t +=
      ...
      TdSkill(
        ...
        EventLimit,
        TalentLimit,
        StationaryCraftItem.length - 1,
        StationaryCraftItem[0],
      );
    for (var i = 1; i < StationaryCraftItem.length; i++) {
      ...
      for (var j = 0; j < StationaryCraftItem[i].length; j += 2) {
        tt += TdMaterial(
          StationaryCraftItem[i][j],
          "×" + StationaryCraftItem[i][j + 1],
          560 / StationaryCraftItem[i].length,
        );
      }
      t += "<td width='580'>" + CompleteTable(tt, 560) + "</td>";
    }
  ...
  try {
    ...
    for (var i = 1; i < FynnsCraftItem.length; i++) {
      ...
      for (var j = 0; j < FynnsCraftItem[i].length; j += 2) {
        tt += TdMaterial(
          FynnsCraftItem[i][j],
          "×" + FynnsCraftItem[i][j + 1],
          560 / FynnsCraftItem[i].length,
        );
      }
      t += "<td width='580'>" + CompleteTable(tt, 560) + "</td>";
    }
  ...
  try {
    ...
    for (var i = 1; i < ManaFormingItem.length; i++) {
      ...
      for (var j = 0; j < ManaFormingItem[i].length; j += 2) {
        tt += TdMaterial(
          ManaFormingItem[i][j],
          "×" + ManaFormingItem[i][j + 1],
          560 / ManaFormingItem[i].length,
        );
      }
      t += "<td width='580'>" + CompleteTable(tt, 560) + "</td>";
    }
  ...
  try {
    ...
    t +=
      ...
      TdSkill(
        ...
        EventLimit,
        TalentLimit,
        MetalConversionItem.length - 1,
        MetalConversionItem[0],
        "",
        OtherSkill,
      );
    for (var i = 1; i < MetalConversionItem.length; i++) {
      if (i != 1) t += "</tr><tr>";
      tt = TdMaterial(
        MetalConversionItem[i][0],
        "×" + MetalConversionItem[i][1] + "～" + MetalConversionItem[i][2],
        560 / MetalConversionItem[i].length,
      );
      t += "<td width='580'>" + CompleteTable(tt, 560) + "</td>";
    }
  } catch (e) {
  ...
  try {
    ...
    t +=
      "</tr><tr>" +
      TdSkill(GatherItem[0], QuestLimit, LocaleLimit, EventLimit) +
      "<td width='580'>" +
      CompleteTable(a + b, 560) +
      "</td>";
    EventLimit = "";
    QuestLimit = "";
  } catch (e) {
  ...
  if (tt != "")
    t +=
      "</tr><tr>" +
      TdSkill(50014, "", "", "", "", RowsQuantity, "", "", OtherSkill) +
      TdText(tt);
  OtherSkill = "";
  tt = "";
  RowsQuantity = 0;
  for (var i = 0; i < RelicInvestigationItemList.length; i++) {
    if (RelicInvestigationItemList[i][1].includes(Id)) {
      ...
      tt +=
        "<td width='580'>" +
        CompleteTable(TdMaterial(RelicInvestigationItemList[i][0], "×1"), 300) +
        "</td>";
      RowsQuantity++;
    }
  }
  if (tt != "")
    t += "</tr><tr>" + TdSkill(57002, "", "", "", "", RowsQuantity) + tt;
  for (var i = 0; i < GiftItemList.length; i++) {
    ...
    for (var j = 1; j < GiftItemList[i].length; j++) {
      if (GiftItemList[i][j][2].includes(Id)) {
        if (tt != "") tt += "</tr><tr>";
        if (GiftItemList[i][j][3] > 1)
          AmendQuantity = "×" + GiftItemList[i][j][3];
        tt +=
          "<td width='580'>" +
          CompleteTable(
            TdMaterial(GiftItemList[i][j][1], AmendQuantity, 170) +
              TdText(GiftItemList[i][j][0]),
            560,
          ) +
          "</td>";
        RowsQuantity++;
    ...
    if (tt != "")
      t +=
        "</tr><tr>" +
        TdSkill(GiftItemList[i][0], "", "", "", "", RowsQuantity) +
        tt;
  }
  ...
  for (var i = 0; i < UseList.length; i++) {
    ...
    if (UseItem[1].includes(Id)) {
      ...
      tt +=
        "<td width='580'>" +
        CompleteTable(TdMaterial(UseList[i]), 560) +
        "</td>";
      RowsQuantity++;
    }
  }
  if (tt != "")
    t += "</tr><tr>" + TdSkill("使用", "", "", "", "", RowsQuantity) + tt;
  if (CheckT == t) {
    t += "</tr><tr>" + TdText("？", 667);
  }
  ...
  BreakLabel: for (let i = 0; i < DissolutionItemList.length; i++) {
    if (DissolutionItemList[i][0] == Id) {
      t =
        "<td title='ClassID：" +
        AmendId(Id) +
        "' class='MainTd'><img border='0' src='img/Item/" +
        AmendId(Id) +
        ".png'><br>" +
        ItemName[0] +
        "<br>×" +
        DissolutionItemList[i][1][0] +
        "</td></tr><tr>" +
        TdSkill(10030, "", "", "", "", 1, DissolutionItemList[i][1][1]);
      for (let j = 0; j < DissolutionItemList[i][2].length; j += 3) {
        tt += TdMaterial(
          DissolutionItemList[i][2][j],
          "×" +
            DissolutionItemList[i][2][j + 1] +
            "～" +
            DissolutionItemList[i][2][j + 2],
          745 / DissolutionItemList[i][2].length,
        );
      }
...
function Product(Id) {
  ...
  if ((Id + "").charAt(3) == 5) {
    ttt =
      "<td colspan=2 style='background-color:gold;color:gold'>" +
      "商团专用" +
      "</td></tr><tr>";
    t0 = "购买";
    Flight = 1;
  ...
  if (Product.length > 11) {
    ...
    tt =
      TdText("交换时需要的材料") +
      "<td>" +
      CompleteTable(tt, 440) +
      "</td></tr><tr>";
  } else {
    t = "<br>信用等级" + Product[2];
    tt =
      TdText("成本价<br><font color='#AAAAAA'>(折扣另算)</font>") +
      "<td>" +
      Product[6] +
      " <font color='#AAAAAA'>(满库存时)</font>～" +
      Product[7] +
      " <font color='#AAAAAA'>(零库存时)</font>" +
      "</td></tr><tr>";
  }
  ...
  t =
    "<td class='MainTd' rowspan=6 width=200><img border='0' src='img/Product/" +
    Id +
    ".png'><br>" +
    Product[0] +
    "</td><td colspan=2>" +
    CompleteTable(
      ttt +
        "<td><img border='0' src='img/Post/" +
        Product[1] +
        ".png'></td><td width=200>" +
        eval("Post" + Product[1] + "[0]") +
        t +
        "</td>",
      200,
    ) +
  ...
  if (Math.floor(Id / 10000) != 2) {
    t +=
      ...
      Capacity(Product[3], Product[4], Flight) +
      "</td>";
  }
...
function Capacity(Weight, MaxBundle, Flight) {
  ...
  for (let i = 1; i < [45, 1002][Flight]; i++) {
    ...
    t1 +=
      "<td onmouseover='ShowTransportData(this," +
      i +
      ")' onmouseout='ShowTransportData(this)'><img border='0' src='img/Transport/" +
      Transport[1] +
      ".png'></td>";
    t2 +=
      "<td>" +
      Math.min(MaxBundle * Transport[2], Math.trunc(Transport[3] / Weight)) +
      "</td>";
    Capacity = Math.min(
      MaxBundle * (Transport[2] + 1),
      Math.trunc((Transport[3] + 100) / Weight),
    );
  ...
  let t =
    "<td height=53></td><td rowspan=3><div class='Capacity' id='Capacity'>" +
    CompleteTable(t1 + t2 + t3, 1152, 1) +
    "</div></td></tr><tr><td>装载量</td></tr><tr><td>装载量<br>宗师级商人</td>";
...
function ErgEnhances(Id) {
  ...
  let tt = "";
  let t =
    "<td style='font-size:30pt' class='MainTd'>" +
    ErgEnhanceItem[0] +
    "</td><td>开放尔格最高等级<br>成功率：" +
    ErgEnhanceItem[1] +
    "%</td></tr><tr>";
  for (var i = 0; i < ErgEnhanceItem[2].length; i++) {
    ...
    if (Limit != "") Limit += "<br>";
    tt += TdMaterial(
      ErgEnhanceItem[2][i][0],
      Limit + "×" + ErgEnhanceItem[2][i][1],
    );
  }
...
function CompleteTable(t, Width = "", Type = 0) {
  ...
  let TableTop =
    "<table cellspacing=0 border=1 align='center' style='text-align:center;' bgColor=black " +
    Width +
    " bordercolor=";
...
function Cuisine(Id) {
  if (CuisineNow != 0)
    document.getElementById("Cuisine" + CuisineNow).className = "";
...
function TdEffect(Item) {
  ...
  for (var i = 1; i < Item.length; i++) {
    ...
    Effect1 +=
      "<td style='border: 1px solid gold;' class='EffectTitle' colspan=" +
      (Effect.length - 1) +
      ">" +
      Effect[0] +
      "</td>";
    for (var j = 1; j < Effect.length; j++) {
      Effect2 +=
        "<td style='border: 1px solid gold;padding: 5px;'>" +
        Effect[j] +
        "</td>";
    }
  }
...
function WandEffect(Effect) {
  ...
  Effect = [
    ["特拉伊", "雷箭", "冰箭", "火箭", "治疗", "打击用"][Effect[1]] + "魔杖",
    Effect1,
    Effect2,
  ];
  return Effect;
...
function CylinderEffect(Effect) {
  ...
  for (var i = 4; i < Effect[2].length; i++) {
    Effect2 +=
      "<font color='#" +
      ["FF0000", "FFFF00", "0000FF", "FFFFFF"][i - 4] +
      "'>" +
      ["火", "土", "水", "风"][i - 4] +
      "炼金术 " +
      Effect[2][i] +
...
function FynnbellEffect(Effect) {
  ...
  Effect1 += eval(
    "`" +
      [
        "菲尼宝珠掉落率 +{0}%",
        "菲恩工艺制作成功时，获得相当于生产经验值 {0}% 的探险经验值",
      ][Effect[1][9]].replace("{0}", "${EffectAmend(Effect[1][10])}") +
      "<br>`",
  );
  let Effect2 = EffectWeapon0(Effect[2]);
...
function EffectArmors(Effect) {
  ...
  Effect0 =
    "防御力 " +
    EffectAmend(Effect[0]) +
    "<br>保护 " +
    EffectAmend(Effect[1]) +
    "<br>" +
    Effect0;
  if (Effect[2]) {
    ...
    Effect0 += eval(
      "`" +
        [
          ...
          ["电磁风暴冷却时间减少{0} 秒", "流星冷却时间减少{0} 秒"],
          ["链式铳炮冷却时间减少{0} 秒", "炼金术结晶保存功能"],
          [
            "连续技伤害增加{0}%",
            "连续技：飞身踢溅射范围增加{0}%",
            "猛袭伤害增加{0}%",
          ],
          ["音乐技能的增益效果{0} 增加", "音乐技能增益效果的持续时间{0}增加"],
          ["人偶操纵术范围增加{0}", "人偶死亡时以{0}% 概率复活"],
          ["", "", "", ""],
          ["", "", "", ""],
        ][Effect[i][0]][Effect[i][1]].replace(
          "{0}",
          " ${EffectAmend(Effect[i+1])}",
        ) +
        "<br>`",
    );
  }
  ...
  for (var i = 2; i < Effect.length; i++) {
    if (Effect[i] != 0) {
      if (Effect[i] > 0) {
        Effect0 +=
          "<br>" +
          "<font color='#0000FF' class='EffectTd'>" +
          EffectName[i - 1] +
          " " +
          Effect[i] +
          " 增加</font>";
        ...
        Effect0 +=
          "<br>" +
          "<font color='#FF0000' class='EffectTd'>" +
          EffectName[i - 1] +
          " " +
          Math.abs(Effect[i]) +
          " 减少</font>";
...
function TdMain(Id, a) {
  ...
  if (a != 1) {
    c =
      ...
      "' onclick='ItemColor(" +
      Id +
      ",this)' style='border:gold solid 5px;'";
    Effect = "";
  }
  let t =
    "<td title='ClassID：" +
    AmendId(Id) +
    "' width=180 rowspan=30" +
    c +
    "><img border='0' src='img/Item/" +
    AmendId(Id) +
    ".png'><br>" +
    ItemName[0] +
    "</td>" +
...
function TdSkill(
  ...
  if (QuestLimit != "")
    t +=
      "<td style='background-color:gold;color:silver' title='" +
      QuestLimit +
      "'>副本限定</td></tr><tr>";
  if (LocaleLimit != "")
    t +=
      "<td style='background-color:gold;color:pink' title='" +
      LocaleLimit +
      "'>区服限定</td></tr><tr>";
  if (EventLimit != "")
    t +=
      "<td style='background-color:gold;color:white' title='" +
      EventLimit +
      "活动'>活动限定</td></tr><tr>";
  if (TalentLimit != "")
    t +=
      "<td style='background-color:gold;color:cyan'>" +
      TalentLimit +
      "专用</td></tr><tr>";
  ...
  if (
    ...
    t +=
      "<td onmouseover='ShowCookingTool(" +
      Lv +
      ",this)' onmouseout='ShowCookingTool()'><img border='0' title='" +
      eval("Skill" + Id + "[0]") +
      " " +
      SkillLv +
      "&#10;" +
      CookingActionNames[Lv] +
      "' src='img/Skill/" +
      Id +
      CookingActionNames[Lv] +
      ".png'></td>";
  ...
  if (OtherSkillId != "")
    t +=
      "</tr><tr><td><img border='0' title='" +
      eval("Skill" + OtherSkillId + "[0]") +
      "' src='img/Skill/" +
      OtherSkillId +
      ".png'></td>";
...
function TdCurrency(Id, Quantity, EventLimit = "") {
  ...
  if (Id == Id + 0) {
    ...
    t +=
      "<td width=80 title='" +
      eval("Item" + Id + "[0]") +
      "' valign='Bottom'><img border='0' src='img/Item/" +
      Id +
      ".png'><br>" +
      Quantity +
      "<br>";
...
function TdMaterial(Id, Proportion = "", Width = 100, Rows = 1) {
  ...
  if (Id == "") {
    ...
    t =
      ...
      "'" +
      Width +
      " valign='Bottom' bgColor=#AAAAAA><div name='Item" +
      Id +
      "'></div>" +
      Proportion +
      "</td>";
...
function TdCharacter(CharacterId, CharacterType) {
  ...
  let t =
    "<td onmouseover='ShowCharacterData(this," +
    CharacterId +
    ",&quot;" +
    CharacterType +
    "&quot;)' onmouseout='ShowCharacterData(this)'><img border='0' src='img/" +
    CharacterType +
    "/" +
    CharacterId +
    ".png'></td>";
...
function ShowCharacterData(th, CharacterId, CharacterType) {
  if (typeof CharacterId != "number") {
    ...
    if (CharacterType == "Pet") {
      CharacterType = "宠物";
      if (
        [730201, 730202, 730206, 730207, 730208, 730209, 730210].includes(
          CharacterId,
        )
      ) {
        CharacterType = "伙伴";
    ...
    if (
      TopPosition + TempHeight >
      parseInt(document.getElementById("MainBody").scrollTop) + 560
    ) {
      TopPosition = AbsolutePosition[1] - 8 - TempHeight;
    }
    document.getElementById("Temp").style.left =
      AbsolutePosition[0] -
      parseInt(document.getElementById("Temp").clientWidth) +
      40 +
      "px";
...
function ShowCookingTool(Lv, th) {
  if (typeof Lv != "number") {
    ...
    document.getElementById("Temp").className = "ShowData";
    t =
      TdMaterial(RighthandCookingTool[CookingToolProject[Lv][0]], "", 80) +
      TdMaterial(LefhandCookingTool[CookingToolProject[Lv][1]], "", 80);
    if ([1, 2, 3, 4, 5, 9, 11, 14].includes(Lv)) {
      t +=
        "</tr><tr><td colspan=2 align='center'>" +
        CompleteTable("<div class='Fire'></div>") +
        "</td>";
    }
    let MainBodyScrollTop = parseInt(
      document.getElementById("MainBody").scrollTop,
    );
    document.getElementById("Temp").innerHTML = CompleteTable(t);
    document.getElementById("Temp").style.left =
      AbsolutePosition[0] + 85 + "px";
    document.getElementById("Temp").style.top =
      Math.max(
        Math.min(
          ...
          MainBodyScrollTop +
            556 -
            parseInt(document.getElementById("Temp").clientHeight),
        ),
        MainBodyScrollTop + 2,
      ) + "px";
  }
...
function ShowTransportData(th, TransportId) {
  if (typeof TransportId != "number") {
    ...
    let t =
      TdText(Transport[0], "", 2) +
      "</tr><tr>" +
      TdText("跑速：" + TransportSpeed[Transport[1] - 1], "", 2);
    ...
    if (Transport.length == 5) {
      for (let i = 0; i < Transport[4].length; i++) {
        ...
        t +=
          "<td><img border='0' src='img/Pet/" +
          Transport[4][i] +
          ".png'></td><td>" +
          eval("Pet" + Transport[4][i] + "[0]") +
          "</td>";
      }
    }
    document.getElementById("Temp").innerHTML = CompleteTable(t);
    ...
    document.getElementById("Temp").style.left =
      Math.max(
        ...
        AbsolutePosition[0] -
          parseInt(document.getElementById("Temp").clientWidth) -
          parseInt(document.getElementById("Capacity").scrollLeft) +
          41,
      ) + "px";
...
function NPCTime() {
  ...
  try {
    document.getElementById("NPC1237").innerHTML =
      SeatNPC1237[Math.floor((Today - 0 + 8 * 60 * 60 * 1000) / 2160000) % 14];
  } catch (e) {
    e;
  }
}
